...
#include <immintrin.h>
#if (defined __AVX__ || defined __SSE4_2__ || (defined(_MSC_VER) && IMGUI_PERF_ENABLE_SIMD_CRC))
...
#define IMGUI_DEBUG_LOG_ERROR(...)        ((void)0)
...
#define IMGUI_DEBUG_LOG_FONT(...)         ((void)0)
#define IMGUI_DEBUG_LOG_INPUTROUTING(...) ((void)0)
#else
...
IMGUI_API ImVec2        ImFontCalcTextSizeEx(ImFont* font, float size, float max_width, float wrap_width, const char* text_begin, const char* text_end_display, const char* text_end, const char** out_remaining, ImVec2* out_offset, ImDrawTextFlags flags, ImFontBaked* baked = NULL);
IMGUI_API const char*   ImFontCalcWordWrapPositionEx(ImFont* font, float size, const char* text, const char* text_end, float wrap_width, ImDrawTextFlags flags = 0, ImFontBaked* baked = NULL);
...
namespace ImGui
{
...
#if IMGUI_PERF_STRICT_OPTIMIZATIONS
    // Opt-in immutable-label entry points. Callers own cache validation and
    // must provide data calculated for the current context/font/ID-stack seed.
    IMGUI_API void          TextExWithPrecomputedSize(const char* text, const char* text_end, const ImVec2& text_size);
    IMGUI_API bool          ButtonExWithPrecomputedLabel(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, ImGuiID id, const char* label_end, const ImVec2& label_size);
    IMGUI_API bool          CheckboxWithPrecomputedLabel(const char* label, bool* value, ImGuiID id, const char* label_end, const ImVec2& label_size);
    IMGUI_API bool          SliderScalarWithPrecomputedLabel(const char* label, ImGuiDataType data_type, void* value, const void* minimum, const void* maximum, const char* format, ImGuiSliderFlags flags, ImGuiID id, const char* label_end, const ImVec2& label_size);
#endif
    ...
    IMGUI_API bool          InputTextExCustom(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback = NULL, void* user_data = NULL);
    ...
    IMGUI_API bool          TempInputTextCustom(const ImRect& bb, ImGuiID id, const char* label, char* buf, size_t buf_size, ImGuiInputTextFlags flags = 0, ImGuiInputTextCallback callback = NULL, void* user_data = NULL);
    IMGUI_API bool          TempInputScalar(const ImRect& bb, ImGuiID id, const char* label, ImGuiDataType data_type, void* p_data, const char* format, const void* p_clamp_min = NULL, const void* p_clamp_max = NULL);
    IMGUI_API bool          TempInputScalarCustom(const ImRect& bb, ImGuiID id, const char* label, ImGuiDataType data_type, void* p_data, const char* format, const void* p_clamp_min = NULL, const void* p_clamp_max = NULL);
