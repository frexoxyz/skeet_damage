...
#include "imgui_impl_dx11_shaders.h"
...
struct ImGui_ImplDX11_StateBackup
{
...
struct ImGui_ImplDX11_Data
{
    ...
    ImVec2                      ProjectionDisplayPos;
    ImVec2                      ProjectionDisplaySize;
    ImVec2                      ProjectionFramebufferScale;
    bool                        ProjectionBufferValid;
    ImGui_ImplDX11_StateBackup StateBackup;

    ImGui_ImplDX11_Data()
    {
        memset((void*)this, 0, sizeof(*this));
        VertexBufferSize = IMGUI_PERF_DX11_INITIAL_VERTEX_CAPACITY;
        IndexBufferSize = IMGUI_PERF_DX11_INITIAL_INDEX_CAPACITY;
    }
};
...
static void ImGui_ImplDX11_SetupRenderState(const ImDrawData* draw_data, ID3D11DeviceContext* device_ctx)
{
    ...
    const bool projection_changed = !bd->ProjectionBufferValid
        || bd->ProjectionDisplayPos.x != draw_data->DisplayPos.x
        || bd->ProjectionDisplayPos.y != draw_data->DisplayPos.y
        || bd->ProjectionDisplaySize.x != draw_data->DisplaySize.x
        || bd->ProjectionDisplaySize.y != draw_data->DisplaySize.y
        || bd->ProjectionFramebufferScale.x != draw_data->FramebufferScale.x
        || bd->ProjectionFramebufferScale.y != draw_data->FramebufferScale.y;
    if (projection_changed)
    {
        ...
        if (device_ctx->Map(bd->pVertexConstantBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mapped_resource) == S_OK)
        {
            ...
            bd->ProjectionDisplayPos = draw_data->DisplayPos;
            bd->ProjectionDisplaySize = draw_data->DisplaySize;
            bd->ProjectionFramebufferScale = draw_data->FramebufferScale;
            bd->ProjectionBufferValid = true;
        }
    }
...
void ImGui_ImplDX11_RenderDrawData(ImDrawData* draw_data)
{
    ...
    if (!bd->pVB || bd->VertexBufferSize < draw_data->TotalVtxCount)
    {
        ...
        const int required_vertex_capacity = draw_data->TotalVtxCount + 5000;
        if (bd->VertexBufferSize < required_vertex_capacity)
            bd->VertexBufferSize = required_vertex_capacity;
    ...
    if (!bd->pIB || bd->IndexBufferSize < draw_data->TotalIdxCount)
    {
        ...
        const int required_index_capacity = draw_data->TotalIdxCount + 10000;
        if (bd->IndexBufferSize < required_index_capacity)
            bd->IndexBufferSize = required_index_capacity;
        D3D11_BUFFER_DESC desc = {};
        desc.Usage = D3D11_USAGE_DYNAMIC;
    ...
    ImGui_ImplDX11_StateBackup& old = bd->StateBackup;
    ...
    ID3D11ShaderResourceView* last_texture_srv = nullptr;
    bool scissor_rect_valid = false;
    bool texture_srv_valid = false;
    for (const ImDrawList* draw_list : draw_data->CmdLists)
    {
        for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            if (pcmd->UserCallback != nullptr)
            {
                ...
                // A callback may change any state we cache between draw commands.
                scissor_rect_valid = false;
                texture_srv_valid = false;
            }
            else
            {
                ...
                if (!scissor_rect_valid || r.left != last_scissor_rect.left || r.top != last_scissor_rect.top ||
                    r.right != last_scissor_rect.right || r.bottom != last_scissor_rect.bottom)
                {
                    device->RSSetScissorRects(1, &r);
                    last_scissor_rect = r;
                    scissor_rect_valid = true;
                }

                ID3D11ShaderResourceView* texture_srv = (ID3D11ShaderResourceView*)pcmd->GetTexID();
                if (!texture_srv_valid || texture_srv != last_texture_srv)
                {
                    device->PSSetShaderResources(0, 1, &texture_srv);
                    last_texture_srv = texture_srv;
                    texture_srv_valid = true;
                }
...
bool    ImGui_ImplDX11_CreateDeviceObjects()
{
    ...
    {
        if (bd->pd3dDevice->CreateVertexShader(g_ImGuiVertexShader, sizeof(g_ImGuiVertexShader), nullptr, &bd->pVertexShader) != S_OK)
            return false;
        ...
        if (bd->pd3dDevice->CreateInputLayout(local_layout, 3, g_ImGuiVertexShader, sizeof(g_ImGuiVertexShader), &bd->pInputLayout) != S_OK)
            return false;
    ...
    {
        if (bd->pd3dDevice->CreatePixelShader(g_ImGuiPixelShader, sizeof(g_ImGuiPixelShader), nullptr, &bd->pPixelShader) != S_OK)
            return false;
    }
...
void    ImGui_ImplDX11_InvalidateDeviceObjects()
{
    ...
    bd->ProjectionBufferValid = false;
