...
#define IMGUI_PERF_INTERNAL_BUILD
...
#if IMGUI_PERF_ENABLE_DRAG_DROP
...
static inline void TextExRenderShort(ImGuiWindow* window, const char* text_begin, const char* text_end, const ImVec2& text_pos, float wrap_width, const ImVec2& text_size)
{
...
void ImGui::TextEx(const char* text, const char* text_end, ImGuiTextFlags flags)
{
    ...
    if (text_end - text <= 2000 || wrap_enabled)
    {
        ...
        TextExRenderShort(window, text_begin, text_end, text_pos, wrap_width, text_size);
    }
    else
    {
...
#if IMGUI_PERF_STRICT_OPTIMIZATIONS
void ImGui::TextExWithPrecomputedSize(const char* text, const char* text_end, const ImVec2& text_size)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
    ...
    const float wrap_pos_x = window->DC.TextWrapPos;
    if (text_end == NULL || text_end - text > 2000 || wrap_pos_x >= 0.0f)
    {
    ...
    const ImVec2 text_pos(window->DC.CursorPos.x, window->DC.CursorPos.y + window->DC.CurrLineTextBaseOffset);
    TextExRenderShort(window, text, text_end, text_pos, 0.0f, text_size);
}
#endif
...
bool ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered, bool* out_held, ImGuiButtonFlags flags)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        else if (g.ActiveIdSource == ImGuiInputSource_Keyboard || (IMGUI_PERF_ENABLE_GAMEPAD && g.ActiveIdSource == ImGuiInputSource_Gamepad))
        {
...
static inline bool ButtonExApplyLabelData(ImGuiWindow* window, ImGuiContext& g, const ImGuiStyle& style, const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, ImGuiID id, const char* label_end, const ImVec2& label_size)
{
    ImVec2 pos = window->DC.CursorPos;
...
#endif
#if IMGUI_PERF_SLIM_WIDGETS && IMGUI_PERF_STRICT_OPTIMIZATIONS
    // label_end is already the rendered end, so avoid scanning the label for "##" a second time.
    if (label != label_end)
        ImGui::RenderTextClippedEx(window->DrawList, bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, label_end, &label_size, style.ButtonTextAlign, &bb);
#else
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    const char* label_end = FindRenderedTextEnd(label);
    const ImVec2 label_size = CalcTextSize(label, label_end, false);
    return ButtonExApplyLabelData(window, g, style, label, size_arg, flags, id, label_end, label_size);
}

#if IMGUI_PERF_STRICT_OPTIMIZATIONS
bool ImGui::ButtonExWithPrecomputedLabel(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, ImGuiID id, const char* label_end, const ImVec2& label_size)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImGuiContext& g = *GImGui;
    return ButtonExApplyLabelData(window, g, g.Style, label, size_arg, flags, id, label_end, label_size);
}
#endif
...
static inline bool CheckboxApplyLabelData(ImGuiWindow* window, ImGuiContext& g, const ImGuiStyle& style, const char* label, bool* v, ImGuiID id, const char* label_end, const ImVec2& label_size)
{
    ...
    if (is_visible)
    {
        ...
        ImU32 bg_col = ImGui::GetColorU32((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : (mixed_value || checked) ? ImGuiCol_CheckboxSelectedBg : ImGuiCol_FrameBg);
    ...
    if (is_visible && label_size.x > 0.0f)
        ImGui::RenderText(label_pos, label, label_end, false);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const char* label_end = FindRenderedTextEnd(label);
    const ImVec2 label_size = CalcTextSize(label, label_end, false);
    return CheckboxApplyLabelData(window, g, g.Style, label, v, id, label_end, label_size);
}

#if IMGUI_PERF_STRICT_OPTIMIZATIONS
bool ImGui::CheckboxWithPrecomputedLabel(const char* label, bool* value, ImGuiID id, const char* label_end, const ImVec2& label_size)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    ImGuiContext& g = *GImGui;
    return CheckboxApplyLabelData(window, g, g.Style, label, value, id, label_end, label_size);
}
#endif
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE* v, float v_speed, const TYPE v_min, const TYPE v_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    else if (g.ActiveIdSource == ImGuiInputSource_Keyboard || (IMGUI_PERF_ENABLE_GAMEPAD && g.ActiveIdSource == ImGuiInputSource_Gamepad))
    {
        ...
        const bool use_gamepad = IMGUI_PERF_ENABLE_GAMEPAD && g.NavInputSource == ImGuiInputSource_Gamepad;
        const bool tweak_slow = IsKeyDown(use_gamepad ? ImGuiKey_NavGamepadTweakSlow : ImGuiKey_NavKeyboardTweakSlow);
        const bool tweak_fast = IsKeyDown(use_gamepad ? ImGuiKey_NavGamepadTweakFast : ImGuiKey_NavKeyboardTweakFast);
...
bool ImGui::DragBehavior(ImGuiID id, ImGuiDataType data_type, void* p_v, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        if (g.ActiveIdSource == ImGuiInputSource_Mouse && !g.IO.MouseDown[0])
            ClearActiveID();
        else if ((g.ActiveIdSource == ImGuiInputSource_Keyboard || (IMGUI_PERF_ENABLE_GAMEPAD && g.ActiveIdSource == ImGuiInputSource_Gamepad)) && g.NavActivatePressedId == id && !g.ActiveIdIsJustActivated)
            ClearActiveID();
    }
    if (g.ActiveId != id)
        return false;
...
static inline bool SliderScalarApplyLabelData(ImGuiWindow* window, ImGuiContext& g, const ImGuiStyle& style, const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags, ImGuiID id, const char* label_end, const ImVec2& label_size, float width, ImU32 color_marker)
{
    ...
    if (color_marker != 0 && style.ColorMarkerSize > 0.0f)
        ImGui::RenderColorComponentMarker(frame_bb, ImGui::GetColorU32(color_marker), style.FrameRounding);
    ...
    if (label_size.x > 0.0f)
        ImGui::RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x, frame_bb.Min.y + style.FramePadding.y), label, label_end, false);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const char* label_end = FindRenderedTextEnd(label);
    const ImVec2 label_size = CalcTextSize(label, label_end, false);
    return SliderScalarApplyLabelData(window, g, g.Style, label, data_type, p_data, p_min, p_max, format, flags, id, label_end, label_size, width, color_marker);
}

#if IMGUI_PERF_STRICT_OPTIMIZATIONS
bool ImGui::SliderScalarWithPrecomputedLabel(const char* label, ImGuiDataType data_type, void* value, const void* minimum, const void* maximum, const char* format, ImGuiSliderFlags flags, ImGuiID id, const char* label_end, const ImVec2& label_size)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    const float width = CalcItemWidth();
    const ImU32 color_marker = (g.NextItemData.HasFlags & ImGuiNextItemDataFlags_HasColorMarker) ? g.NextItemData.ColorMarker : 0;
    return SliderScalarApplyLabelData(window, g, g.Style, label, data_type, value, minimum, maximum, format, flags, id, label_end, label_size, width, color_marker);
}
#endif
...
bool ImGui::TempInputTextCustom(const ImRect& bb, ImGuiID id, const char* label, char* buf, size_t buf_size, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* user_data)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    ...
    window->DC.CursorPos = bb.Min;
    g.LastItemData.ItemFlags |= ImGuiItemFlags_AllowDuplicateId;
    bool value_changed = InputTextExCustom(label, NULL, buf, (int)buf_size, bb.GetSize(), flags | ImGuiInputTextFlags_TempInput | ImGuiInputTextFlags_AutoSelectAll, callback, user_data);
    KeepAliveID(id);
    if (init)
    {
        IM_ASSERT(g.ActiveId == id);
...
bool ImGui::TempInputScalarCustom(const ImRect& bb, ImGuiID id, const char* label, ImGuiDataType data_type, void* p_data, const char* format, const void* p_clamp_min, const void* p_clamp_max)
{
    ImGuiContext& g = *GImGui;
    const ImGuiDataTypeInfo* type_info = DataTypeGetInfo(data_type);
    char fmt_buf[32];
    ...
    ImGuiInputTextFlags flags = ImGuiInputTextFlags_AutoSelectAll | (ImGuiInputTextFlags)ImGuiInputTextFlags_LocalizeDecimalPoint;
    g.LastItemData.ItemFlags |= ImGuiItemFlags_NoMarkEdited;
    if (!TempInputTextCustom(bb, id, label, data_buf, IM_COUNTOF(data_buf), flags))
        return false;
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (g.ActiveId == id && !g.ActiveIdIsJustActivated && !clear_active_id)
    {
        ...
        const bool is_copy = false;
        const bool is_paste = false;
#endif
        ...
        const bool nav_gamepad_active = IMGUI_PERF_ENABLE_GAMEPAD && (io.ConfigFlags & ImGuiConfigFlags_NavEnableGamepad) != 0 && (io.BackendFlags & ImGuiBackendFlags_HasGamepad) != 0;
    ...
    if (render_cursor)
    {
        ...
        if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
            draw_window->DrawList->AddLineV(cursor_screen_rect.Min.x, cursor_screen_rect.Min.y, cursor_screen_rect.Max.y, GetColorU32(ImGuiCol_InputTextCursor), 1.0f * (float)(int)style._MainScale);
...
bool ImGui::InputTextExCustom(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (render_cursor || render_selection)
    {
        ...
        {
            struct scroll_anim_slot_t { float displayed_x = 0.f; bool inited = false; };
            static ImVector<scroll_anim_slot_t> s_scroll_slots;
            static ImVector<ImGuiID>            s_scroll_slot_ids;
            int sc_idx = -1;
            for (int i = 0; i < s_scroll_slot_ids.Size; i++) if (s_scroll_slot_ids[i] == id) { sc_idx = i; break; }
            if (sc_idx == -1) { sc_idx = s_scroll_slot_ids.Size; s_scroll_slot_ids.push_back(id); s_scroll_slots.resize(s_scroll_slot_ids.Size); }
            scroll_anim_slot_t& scroll_slot = s_scroll_slots[sc_idx];
            if (!scroll_slot.inited) { scroll_slot.displayed_x = state->Scroll.x; scroll_slot.inited = true; }
            if (ImFabs(scroll_slot.displayed_x - state->Scroll.x) > inner_size.x)
                scroll_slot.displayed_x = state->Scroll.x;
            scroll_slot.displayed_x = ImLerp(scroll_slot.displayed_x, state->Scroll.x, 1.f - expf(-22.f * io.DeltaTime));
            draw_scroll.x = scroll_slot.displayed_x;
        }
        if (render_selection)
        {
            struct sel_alpha_slot_t { float alpha = 0.f; };
            static ImVector<sel_alpha_slot_t> s_sel_slots;
            static ImVector<ImGuiID>          s_sel_slot_ids;
            int sel_idx = -1;
            for (int i = 0; i < s_sel_slot_ids.Size; i++) if (s_sel_slot_ids[i] == id) { sel_idx = i; break; }
            if (sel_idx == -1) { sel_idx = s_sel_slot_ids.Size; s_sel_slot_ids.push_back(id); s_sel_slots.resize(s_sel_slot_ids.Size); }
            sel_alpha_slot_t& sslot = s_sel_slots[sel_idx];
            const float sel_target = render_selection ? 1.f : 0.f;
            sslot.alpha = ImLerp(sslot.alpha, sel_target, 1.f - expf(-16.f * io.DeltaTime));

            if (sslot.alpha > 0.005f)
            {
                ImVec4 sel_col4 = ColorConvertU32ToFloat4(GetColorU32(ImGuiCol_TextSelectedBg, render_cursor ? 1.0f : 0.6f));
                sel_col4.w *= sslot.alpha;
                const ImU32 bg_color = GetColorU32(sel_col4);
                const float bg_offy_up = is_multiline ? 0.0f : -1.0f;
                const float bg_offy_dn = is_multiline ? 0.0f : 2.0f;
                const float bg_eol_width = IM_TRUNC(g.FontBaked->GetCharAdvance((ImWchar)' ') * 0.50f);
                const float sel_round = is_multiline ? 0.f : 2.f;
                ...
                for (int line_n = line_visible_n0; line_n < line_visible_n1; line_n++)
                {
                    ...
                    rect.Min.y += bg_offy_up;
                    rect.ClipWith(clip_rect);
                    draw_window->DrawList->AddRectFilled(rect.Min, rect.Max, bg_color, sel_round);
                }
            }
        }
    }
    ...
    if ((is_multiline || (buf_display_end - buf_display) < buf_display_max_length) && (text_col & IM_COL32_A_MASK) && (line_visible_n0 < line_visible_n1))
    {
        struct typing_fade_slot_t { int last_len = -1; int faded_bytes = 0; float t = 1.f; };
        static ImVector<typing_fade_slot_t> s_type_slots;
        static ImVector<ImGuiID>            s_type_slot_ids;
        int t_idx = -1;
        for (int i = 0; i < s_type_slot_ids.Size; i++) if (s_type_slot_ids[i] == id) { t_idx = i; break; }
        if (t_idx == -1) { t_idx = s_type_slot_ids.Size; s_type_slot_ids.push_back(id); s_type_slots.resize(s_type_slot_ids.Size); }
        typing_fade_slot_t& tslot = s_type_slots[t_idx];

        const int cur_byte_len = (int)(buf_display_end - buf_display);
        if (tslot.last_len < 0) tslot.last_len = cur_byte_len;
        if (cur_byte_len > tslot.last_len) {
            tslot.faded_bytes = cur_byte_len - tslot.last_len;
            tslot.t = 0.f;
        } else if (cur_byte_len < tslot.last_len) {
            tslot.faded_bytes = 0;
            tslot.t = 1.f;
        }
        tslot.last_len = cur_byte_len;
        tslot.t = ImLerp(tslot.t, 1.f, 1.f - expf(-18.f * io.DeltaTime));

        const ImVec2 text_pos = draw_pos - draw_scroll + ImVec2(0.0f, line_visible_n0 * g.FontSize);
        const char*  line0_begin = line_index->get_line_begin(buf_display, line_visible_n0);
        const char*  lineN_end   = line_index->get_line_end(buf_display, line_visible_n1 - 1);

        if (!is_multiline && tslot.t < 0.999f && tslot.faded_bytes > 0 && tslot.faded_bytes <= (int)(lineN_end - line0_begin))
        {
            const char* split = lineN_end - tslot.faded_bytes;
            if (split > line0_begin) {
                g.Font->RenderText(draw_window->DrawList, g.FontSize, text_pos,
                    text_col, clip_rect.AsVec4(), line0_begin, split,
                    wrap_width, ImDrawTextFlags_WrapKeepBlanks | ImDrawTextFlags_CpuFineClip);
            }
            const float pre_w = CalcTextSize(line0_begin, split).x;
            ImVec4 fade_col4 = ColorConvertU32ToFloat4(text_col);
            fade_col4.w *= tslot.t;
            const ImU32 fade_col = GetColorU32(fade_col4);
            g.Font->RenderText(draw_window->DrawList, g.FontSize, ImVec2(text_pos.x + pre_w, text_pos.y),
                fade_col, clip_rect.AsVec4(), split, lineN_end,
                wrap_width, ImDrawTextFlags_WrapKeepBlanks | ImDrawTextFlags_CpuFineClip);
        }
        else
        {
            g.Font->RenderText(draw_window->DrawList, g.FontSize, text_pos,
                text_col, clip_rect.AsVec4(), line0_begin, lineN_end,
                wrap_width, ImDrawTextFlags_WrapKeepBlanks | ImDrawTextFlags_CpuFineClip);
        }
    }
    ...
    if (render_cursor)
    {
        ...
        struct cursor_anim_slot_t { float x = -1.f; int last_pos = -1; double last_move = 0.0; };
        static ImVector<cursor_anim_slot_t> s_cursor_slots;
        static ImVector<ImGuiID>            s_cursor_slot_ids;
        int slot_idx = -1;
        for (int i = 0; i < s_cursor_slot_ids.Size; i++) if (s_cursor_slot_ids[i] == id) { slot_idx = i; break; }
        if (slot_idx == -1) { slot_idx = s_cursor_slot_ids.Size; s_cursor_slot_ids.push_back(id); s_cursor_slots.resize(s_cursor_slot_ids.Size); }
        cursor_anim_slot_t& slot = s_cursor_slots[slot_idx];

        const ImVec2 cursor_screen_pos = ImTrunc(draw_pos + cursor_offset - draw_scroll);
        if (slot.x < 0.f) slot.x = cursor_screen_pos.x;
        slot.x = ImLerp(slot.x, cursor_screen_pos.x, 1.f - expf(-22.f * io.DeltaTime));
        if (slot.last_pos != state->Stb->cursor) { slot.last_pos = state->Stb->cursor; slot.last_move = g.Time; }

        const double since_move = g.Time - slot.last_move;
        float blink;
        if (since_move < 0.4) {
            blink = 1.f;
        } else {
            const float w = sinf((float)g.Time * 4.f) * 0.5f + 0.5f;
            blink = 0.25f + w * 0.75f;
        }
        if (!g.IO.ConfigInputTextCursorBlink) blink = 1.f;

        ImVec2 cursor_a(slot.x, cursor_screen_pos.y - g.FontSize + 0.5f);
        ImVec2 cursor_b(slot.x, cursor_screen_pos.y - 1.5f);
        ImRect cursor_screen_rect(cursor_a.x, cursor_a.y, cursor_a.x + 1.f, cursor_b.y);
        if (cursor_screen_rect.Overlaps(clip_rect))
        {
            ImVec4 cur_col = ColorConvertU32ToFloat4(GetColorU32(ImGuiCol_InputTextCursor));
            cur_col.w *= blink;
            draw_window->DrawList->AddLineV(cursor_a.x, cursor_a.y, cursor_b.y, GetColorU32(cur_col), 1.0f * (float)(int)style._MainScale);
        }
        ...
        if (!is_readonly && g.ActiveId == id)
        {
            ...
            ime_data->WantVisible = true;
            ime_data->WantTextInput = true;
            ime_data->InputPos = ImVec2(cursor_a.x - 1.0f, cursor_screen_pos.y - g.FontSize);
            ime_data->InputLineHeight = g.FontSize;
            ime_data->ViewportId = window->Viewport->ID;
        }
    }
...
void ImGui::ColorEditOptionsPopup(const float* col, ImGuiColorEditFlags flags)
{
    ...
    {
#if IMGUI_PERF_SLIM_WIDGETS
        const ImGuiSelectableFlags selectable_flags = ImGuiSelectableFlags_NoAutoClosePopups;
        if (Selectable("RGB", (opts & ImGuiColorEditFlags_DisplayRGB) != 0, selectable_flags)) opts = (opts & ~ImGuiColorEditFlags_DisplayMask_) | ImGuiColorEditFlags_DisplayRGB;
        if (Selectable("HSV", (opts & ImGuiColorEditFlags_DisplayHSV) != 0, selectable_flags)) opts = (opts & ~ImGuiColorEditFlags_DisplayMask_) | ImGuiColorEditFlags_DisplayHSV;
        if (Selectable("Hex", (opts & ImGuiColorEditFlags_DisplayHex) != 0, selectable_flags)) opts = (opts & ~ImGuiColorEditFlags_DisplayMask_) | ImGuiColorEditFlags_DisplayHex;
#else
...
#if IMGUI_PERF_SLIM_WIDGETS
        const ImGuiSelectableFlags selectable_flags = ImGuiSelectableFlags_NoAutoClosePopups;
        if (Selectable("0..255", (opts & ImGuiColorEditFlags_Uint8) != 0, selectable_flags)) opts = (opts & ~ImGuiColorEditFlags_DataTypeMask_) | ImGuiColorEditFlags_Uint8;
        if (Selectable("0.00..1.00", (opts & ImGuiColorEditFlags_Float) != 0, selectable_flags)) opts = (opts & ~ImGuiColorEditFlags_DataTypeMask_) | ImGuiColorEditFlags_Float;
#else
...
void ImGui::ColorPickerOptionsPopup(const float* ref_col, ImGuiColorEditFlags flags)
{
    ...
    if (allow_opt_alpha_bar)
    {
        ...
        bool alpha_bar = (g.ColorEditOptions & ImGuiColorEditFlags_AlphaBar) != 0;
        if (Checkbox("Alpha Bar", &alpha_bar))
            g.ColorEditOptions = alpha_bar ? (g.ColorEditOptions | ImGuiColorEditFlags_AlphaBar) : (g.ColorEditOptions & ~ImGuiColorEditFlags_AlphaBar);
    }
    PopItemFlag();
    EndPopup();
}
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
...

    {
        ...
        {
            if (label != label_end)
                RenderTextClippedEx(window->DrawList, text_pos, frame_bb.Max, label, label_end, &label_size);
        }
#else
...
// Error recovery in imgui.cpp still references TreePop(). Slim builds don't expose
// tree widgets, so only keep the minimum stack unwinding path required by recovery.
void ImGui::TreePop()
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    if (is_visible)
    {
        ...
        // label_end is already the rendered end, so avoid scanning the label for "##" a second time.
        if (label != label_end)
            RenderTextClippedEx(window->DrawList, pos, ImVec2(ImMin(pos.x + size.x, window->WorkRect.Max.x), pos.y + size.y), label, label_end, &label_size, style.SelectableTextAlign, &bb);
#else
...
// Retained only for imgui.cpp error recovery. BeginMultiSelect() is unavailable in
// slim builds, so this path normally has no live state to unwind.
ImGuiMultiSelectIO* ImGui::EndMultiSelect()
{
    ImGuiContext& g = *GImGui;
    ImGuiMultiSelectTempData* ms = g.CurrentMultiSelect;
    ...
    if (g.MultiSelectTempDataStacked > 0)
...
bool    ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, ImGuiWindow* docked_window)
{
    ...
    if (is_visible)
    {
        ...
        bool text_clipped;
#endif
        TabItemLabelAndCloseButton(display_draw_list, bb, tab_just_unsaved ? (flags & ~ImGuiTabItemFlags_UnsavedDocument) : flags, tab_bar->FramePadding, label, id, close_button_id, tab_contents_visible, &just_closed,
#if IMGUI_PERF_ENABLE_TOOLTIPS
            &text_clipped
#else
            NULL
